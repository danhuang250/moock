<template>
  <el-dialog width="50%" :model-value="courseDataVisible" destroy-on-close :close-on-click-modal="false"
    @close="onCancel">
    ...
    <el-table v-loading="tableLoading" row-key="id" header-row-class-name="custom-table-header" :data="tableData">
      ...
      <el-table-column label="下载地址" prop="value">
        <template #default="{ row }">
      ...
      <el-table-column label="操作" prop="action" width="200">
        <template #default="{ row }">
          ...
          <span v-else style="display: flex;justify-content:space-around;">
            ...
            <!--上传资料 start-->
            <el-upload ref="uploadRef" class="upload-demo" name="fileResource" :on-error="uploadError"
              :action="uploadURL" :before-upload="beforeUpload" :limit="1" :data="{ courseDataId: row.id, fileKey: fileKey }"
              :show-file-list="false" :on-success="FileOnSuccess">
