...
export const staticRouter = [
    {
        path: '/',
        meta: { title: '蛋黄网-个人开发者创业专用一站式在线教育平台' },
        isMenu: false,
...
router.beforeEach(async (to, from, next) => {
    ...
    if (typeof (to.meta.title) === 'string') {
        document.title = to.meta.title || '蛋黄网-个人开发者创业专用一站式在线教育平台'
    }
        ...
        if (to.path.indexOf('edu') !== -1) {
            ...
            //     // 根据角色动态生成路由访问映射
                ...
                if(!menuStore.routers.length){
                    const accessRoutes = menuStore.generateRouter({roles: ["ROLE_ADMIN"]})
